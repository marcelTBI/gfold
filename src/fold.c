...
PUBLIC double  fold(const char *string, char *structure);
PUBLIC double  energy_of_struct(const char *string, const char *structure);
...
PRIVATE int   *fS;      /*energy of sequence S */
PRIVATE int   *fcS;      /*energy of sequence S */
PRIVATE int   *fMLS;      /*energy of sequence S */
PRIVATE int   *fR;      /*energy of sequence R */
PRIVATE int   *fcR;      /*energy of sequence R */
PRIVATE int   *fMLR;      /*energy of sequence R */
PRIVATE int   *ftemp;   /*full version of energy of sequence */
...
PRIVATE void get_arrays(unsigned int size)
{
  ...
  ftemp = (int *) space(sizeof(int)*((size*(size+1))/2+2));
...
double fold(const char *string, char *structure) {
  ...
  if (backtrack_type=='C')
    return (double) c[indx[length]+1]/100.;
  else if (backtrack_type=='M')
    return (double) fML[indx[length]+1]/100.;
  else
...
PRIVATE int fill_arrays(const char *string) {
	...
	if (!no_close) {
	  ...
	  if (dangles) {
	    ...
	    if (dangles==2) /* double dangles */
	      ...
	      decomp = MIN2(DMLi2[j-1]+d3+P->MLbase, decomp); //FML(i+2,j-1)
	      decomp = MIN2(DMLi1[j-2]+d5+P->MLbase, decomp); //FML(i+1,j-2)
	      decomp = MIN2(DMLi2[j-2]+d5+d3+2*P->MLbase, decomp); // FML(i+2.j-2)
	    }
	  }
  ...
  int row;
  for (row=1;row<=length-TURN-1;row++) {
  ftemp[indx[TURN]+row]=0;
  for (j=TURN+row+1; j<=length; j++) {
    ftemp[indx[j]+row] = ftemp[indx[j-1]+row];
    type=ptype[indx[j]+row];
    if (type) {
      energy = c[indx[j]+row];
      if (type>2) energy += P->TerminalAU;
      if ((dangles==2)&&(j<length))  /* double dangles */
	energy += P->dangle3[type][S1[j+1]];
      ftemp[indx[j]+row] = MIN2(ftemp[indx[j]+row], energy);
    }
    type=ptype[indx[j-1]+row];
    if ((type)&&(dangles%2==1)) {
      energy = c[indx[j-1]+row]+P->dangle3[type][S1[j]];
      if (type>2) energy += P->TerminalAU;
      ftemp[indx[j]+row] = MIN2(ftemp[indx[j]+row], energy);
    }
    for (i=j-TURN-1; i>row; i--) { /*----------------------------------*/
      type = ptype[indx[j]+i];
      if (type) {
	energy = ftemp[indx[i-1]+row]+c[indx[j]+i];
	if (type>2) energy += P->TerminalAU;
	if (dangles==2) {
	...
	ftemp[indx[j]+row] = MIN2(ftemp[indx[j]+row], energy);
	if (dangles%2==1) {
	  energy = ftemp[indx[i-2]+row]+c[indx[j]+i]+P->dangle5[type][S1[i-1]];
	  if (type>2) energy += P->TerminalAU;
	  ftemp[indx[j]+row] = MIN2(ftemp[indx[j]+row], energy);
	}
      }
	...
	if (type>2) energy += P->TerminalAU;
	ftemp[indx[j]+row] = MIN2(ftemp[indx[j]+row], ftemp[indx[i-1]+row]+energy);
	ftemp[indx[j]+row] = MIN2(ftemp[indx[j]+row],
 ftemp[indx[i-2]+row]+energy+P->dangle5[type][S1[i-1]]);
      }
    }
  }
  }
  row=1;
  for (i=0;i<=TURN+row;i++)
    f5[i]=0;
  for (j=TURN+2;j<=length;j++)
    f5[j]=ftemp[indx[j]+row];
  return f5[length];
}
...
double energy_of_struct(const char *string, const char *structure)
{
  int   energy;
  short *ss, *ss1;
...
double energy_of_circ_struct(const char *string, const char *structure) {
...
PRIVATE int *InterAct; // tight structure including interaction 
PRIVATE int *Tight; // tight structure
PRIVATE int *Tul; // tight structure with upper and lower arcs
PRIVATE int *Tl; // tight structure with upper arc only
PRIVATE int *Tu; // tight structure with lower arc only
PRIVATE int *RT; // right tied structure
PRIVATE int *IAS; // arbitray structure with at least one interaction
PRIVATE int *AS; // arbitray structure 
PRIVATE int *InterAct;
PRIVATE int *indx1,*indx2;
PRIVATE short S_s[10000],S_r[10000];

PRIVATE void get_arrays_co(unsigned int sizeS, unsigned int sizeR) 
{
  InterAct = (int *) space(sizeof(int)*(((sizeS*(sizeS+1))/2+2)*(sizeR*(sizeR+1)/2+2)));
  Tight=(int *) space(sizeof(int)*(((sizeS*(sizeS+1))/2+2)*(sizeR*(sizeR+1)/2+2)));
  Tul = (int *) space(sizeof(int)*(((sizeS*(sizeS+1))/2+2)*(sizeR*(sizeR+1)/2+2)));
  Tl  = (int *) space(sizeof(int)*(((sizeS*(sizeS+1))/2+2)*(sizeR*(sizeR+1)/2+2)));
  Tu  = (int *) space(sizeof(int)*(((sizeS*(sizeS+1))/2+2)*(sizeR*(sizeR+1)/2+2)));
  RT  = (int *) space(sizeof(int)*(((sizeS*(sizeS+1))/2+2)*(sizeR*(sizeR+1)/2+2)));
  IAS = (int *) space(sizeof(int)*(((sizeS*(sizeS+1))/2+2)*(sizeR*(sizeR+1)/2+2)));
  AS  = (int *) space(sizeof(int)*(((sizeS*(sizeS+1))/2+2)*(sizeR*(sizeR+1)/2+2)));
  indx1 = (int *) space(sizeof(int)*(sizeS+1));
  indx2 = (int *) space(sizeof(int)*(sizeS+1));
  fS  = (int *) space(sizeof(int)*((sizeS*(sizeS+1)/2+2)));
  fcS = (int *) space(sizeof(int)*((sizeS*(sizeS+1)/2+2)));
  fMLS= (int *) space(sizeof(int)*((sizeS*(sizeS+1)/2+2)));
  fR  = (int *) space(sizeof(int)*((sizeR*(sizeR+1)/2+2)));
  fcR = (int *) space(sizeof(int)*((sizeR*(sizeR+1)/2+2)));
  fMLR= (int *) space(sizeof(int)*((sizeR*(sizeR+1)/2+2)));

}

PRIVATE void free_arrays_co(void)
{
  free(InterAct);
  free(indx1);free(indx2);
  free(Tul); free(Tu); free(Tl);
  free(RT); free(AS); free(IAS); free(Tight);
}

inline int LocateIndex(int i,int j,int i1,int j1,int lengthR)
{
	return (indx1[j]+i)*(lengthR*(lengthR+1)/2)+indx2[j1]+i1;
}

void initialize_fold_co(int lengthS, int lengthR)
{
  unsigned int ns,nr;
  if ((lengthS<1)||(lengthR<1)) nrerror("initialize_fold: argument must be greater 0");
  get_arrays_co((unsigned) lengthS, (unsigned) lengthR);

  for (nr = 1; nr <= (unsigned) lengthR; nr++)
    indx2[nr] = (nr*(nr-1)) >> 1;        /* nr(nr-1)/2 */

  for (ns = 1; ns <= (unsigned) lengthS; ns++)
    indx1[ns] = (ns*(ns-1)) >> 1;        /* ns(ns-1)/2 */

  update_fold_params();
}

PRIVATE void encode_seq_co(const char *stringS, const char *stringR) {

  unsigned int i,ls,lr;
  ls = strlen(stringS); lr=strlen(stringR);
  S_s[0] = (short) ls;
  S_r[0] = (short) lr;

  for (i=1; i<=ls; i++) { /* make numerical encoding of sequence S */
    S_s[i]= (short) encode_char(toupper(stringS[i-1]));
  }
  for (i=1; i<=lr; i++) { /* make numerical encoding of sequence R */
    S_r[i]= (short) encode_char(toupper(stringR[i-1]));
  }
}
